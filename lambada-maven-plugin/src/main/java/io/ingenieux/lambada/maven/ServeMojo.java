...
public class ServeMojo extends AbstractLambadaMetadataMojo {
    ...
    properties
        ...
        .forEach(
            entry -> {
              ...
              if (!matcherKeyVariable.matches()) return;
  ...
  public void loadPathHandlers() throws Exception {
    ...
    for (Method m : methodsAnnotatedWith) {
      ...
      if (null == annotation.api() || 1 != annotation.api().length) return;
  ...
  class PathHandler implements Comparable<PathHandler> {
    ...
    public String toString() {
      return "PathHandler{" + "path='" + path + '\'' + ", methodType=" + methodType + ", instance=" + instance + ", method=" + method + '}';
    }
    ...
    public PathHandler build() {
      ...
      if (null == invoker.getUserHandler()) throw new IllegalArgumentException("Oops: Unable to build an path handler for " + this.toString());
  ...
  private PassthroughRequest<JsonNode> buildPassthroughtRequest(Request request) throws Exception {
    ...
    boolean looksLikePassthrough =
        asList("body-json", "params", "stage-variables", "context")
                .stream()
                .map(
                    x -> {
